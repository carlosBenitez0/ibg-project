...
export const Navbar = ({ inHeroSection = false }: NavbarProps) => {
  ...
  return (
    <nav
      className={`${inHeroSection ? 'absolute top-0 z-60' : 'relative z-50'} flex w-full items-center justify-center px-4 py-4 text-white sm:px-6`}
    >
      ...
      >
        ...
        <aside
          ...
          className={`absolute right-0 top-0 flex h-full w-[min(88vw,34rem)] flex-col bg-white text-slate-800 shadow-2xl shadow-black/30 transition-transform duration-300 ease-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-5 sm:px-6">
            {/* <IbgLogoPersonalizedText textColorClassName="text-slate-800" /> */}
